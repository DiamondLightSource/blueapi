...
def test_start_worker_raises_if_already_started():
    main.start_worker(ApplicationConfig())
    with pytest.raises(main.InitialisationException):
        main.start_worker(ApplicationConfig())
...
def test_get_plans():
    ...
    main.start_worker(ApplicationConfig(), bluesky_context=context)

    assert main.get_plans() == [
        PlanModel(
            name="my_plan",
...
def test_get_plan():
    ...
    assert main.get_plan("my_plan") == PlanModel(
        name="my_plan",
        description="My plan does cool stuff.",
        parameter_schema={
            "title": "my_plan",
    ...
    with pytest.raises(KeyError):
        main.get_plan("non_existing_plan")
...
def test_get_device():
    ...
    context.device(MyDevice(name="my_device"))
    main.start_worker(ApplicationConfig(), bluesky_context=context)
    assert main.get_device("my_device") == DeviceModel(
        name="my_device", protocols=["HasName"]
    )

    with pytest.raises(KeyError):
        assert main.get_device("non_existing_device")
...
def test_submit_task():
    ...
    with patch.object(uuid, "uuid4") as uuid_mock:
        ...
        task_uuid = main.submit_task(task)
...
def test_clear_task():
    ...
    with patch.object(uuid, "uuid4") as uuid_mock:
        uuid_mock.return_value = uuid.UUID(mock_uuid_value)
        main.submit_task(task)

    clear_task_return = main.clear_task(mock_uuid_value)
...
def test_begin_task(worker_mock: MagicMock):
    ...
    returned_task = main.begin_task(task)
...
def test_get_tasks_by_status(get_tasks_by_status_mock: MagicMock):
    ...
    assert main.get_tasks_by_status(TaskStatusEnum.PENDING) == [
        pending_task1,
        pending_task2,
    ]
    assert main.get_tasks_by_status(TaskStatusEnum.RUNNING) == [running_task]
    assert main.get_tasks_by_status(TaskStatusEnum.COMPLETE) == []


def test_get_active_task():
    main.start_worker(ApplicationConfig())
    assert main.get_active_task() is None


def test_get_worker_state():
    main.start_worker(ApplicationConfig())
    assert main.get_worker_state() == WorkerState.IDLE


@patch("blueapi.service.worker.main.TaskWorker.pause")
def test_pause_worker(pause_worker_mock: MagicMock):
    main.start_worker(ApplicationConfig())
    main.pause_worker(False)
    pause_worker_mock.assert_called_once_with(False)

    pause_worker_mock.reset_mock()
    main.pause_worker(True)
...
def test_cancel_active_task(cancel_active_task_mock: MagicMock):
    ...
    assert main.cancel_active_task(fail, reason) == task_id
...
def test_get_task_by_id():
    ...
    task_id = main.submit_task(Task(name="my_plan"))

    assert main.get_task_by_id(task_id) == TrackableTask(
        task_id=task_id,
