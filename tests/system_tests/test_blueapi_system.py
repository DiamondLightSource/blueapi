...
#
# 2. Build and run OPA (Open Policy Agent)
# (Ensure opa is in your system PATH)
# Build the OPA data bundle:
# opa build -b tests/system_tests/services/opa_config/opa_data
#
# Start the OPA server with the bundle:
# export ISSUER=http://localhost:8081/realms/master
# opa run --server --addr :8181 -b ./bundle.tar.gz \
#   --config-file ./tests/system_tests/services/opa_config/config.yaml
#
# 3. Start Tiled server
# docker run -d --name tiled --network host \
# -v "./tests/system_tests/services/tiled_config:/deploy/config" \
# ghcr.io/zohebshaikh/tiled:0.4.17 tiled serve config --host localhost --port 8407
#
# 4. Spin up blueapi server (inside devcontainer)
#
