...
As well as running plans through the CLI, it is possible to run plans using the docs page. Below are instructions to do the same for the pxx-training rigs.

:::{note}
 As a prerequisite to this tutorial, you should have access to the pollux namespace and p-xx training rig namespaces. Visit [this link](https://jira.diamond.ac.uk/servicedesk/customer/portal/2/create/92) to request access.
:::
